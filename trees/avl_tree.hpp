...
private:
	void rotateLeft(BinTreeMutablePosition<AVLElement<T>> x) {
		...
		BinTreeMutablePosition<AVLElement<T>> y = tmp.rootMutablePos();
	...
	void rotateRight(BinTreeMutablePosition<AVLElement<T>> y) {
		BinTree<AVLElement<T>> tmp;
		BinTreeMutablePosition<AVLElement<T>> x = tmp.rootMutablePos();
		BinTree<AVLElement<T>>::assignFrom(x, -y);
	...
	int insertAt(BinTreeMutablePosition<AVLElement<T>> p, T const& x) {
		...
		if (!p) {
			...
			BinTree<AVLElement<T>>::assignFrom(p, tree.rootMutablePos());
			return 1;
		}
...
public:
	...
	BinTreeMutablePosition<AVLElement<T>> search(T const& x) {
	...
	bool insert(T const& x) {
		return insertAt(BSTree<AVLElement<T>>::rootMutablePos(), x) >= 0;
	}
